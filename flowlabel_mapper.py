...
with open(flow_label,'r') as f:
    ...
    for row in data:
        ...
        # print(f"\nThese are individual files {file}")
        sid,cat = int(row[hidx['sid']]),int(row[hidx['category']]) # --> cat is category
        if sid in S: S[sid] += [file] # S stores the files
        else:        S[sid]  = [file]
        if cat in C: C[cat] += [file] # C stores the files 
        else:        C[cat]  = [file]
        if sid in SC:
...
for c in C:
    ...
    for i in range(len(C[c])):
        cwd = os.getcwd()
        in_path  = cwd+"/"+in_dir+str(C[c][i])
        # print(f"\nThese are individual files {in_path}")
...
print('%s image paths were not mapped to a label and were not moved')
