...
COMMENT: Seems to be the main image processor/filtering
"""
...
"""
FOR JUN: THIS IS FOR DETECTING AND SEPARATING IMAGES

DESCRIPTION: given a sid and deployment (label), 
look for events: (chroma dropped, camera moved, 
lens flare, moisture blur)
...
def skip_to_ref(paths,width,height,verbose=True):
    i,ref = 0,None # i is index and ref is reference image
    if len(paths)>0:
    ...
    return ref
    # FOR FINDING GOOD REFERENCE IMAGE IS ONLY CHECKING IF CROMMA_DROPPED
...
"""
Continuation of detect_events, PROCESS IMAGE LOCATED HERE
"""
def process_image_partitions(C,params):                                        # C are images
    width,height         = params['width'],params['height']
    enh_hrs,agg_hrs      = params['enh_hrs'],params['agg_hrs']                 # EXAMINE THIS
    equalize,advanced    = params['equalize'],params['advanced']               # EXAMINE THIS
    mean,sharp,winsize   = params['mean'],params['sharp'],params['winsize']    # EXAMINE THIS
    pad,d_min,pixel_dist = params['pad'],params['d_min'],params['pixel_dist']  # EXAMINE THIS
    write_labels         = params['write_labels']                              
    for sid in C:                     # C contains sids
        for deploy in sorted(C[sid]): # C[sid] contains deploy
            n = len(C[sid][deploy])   # calculates the number of paths 
            print('processing %s paths for sid=%s, deploy=%s'%(n,sid,deploy))

            # [1] find a starting reference image po+int::::::::::::::::::::::::::::::::::::::::::::::::::::::::
            ref = skip_to_ref([e[-1] for e in C[sid][deploy]],width=width,height=height) # [datetime, label, camera,path]
                # finds a reference image point based on the last image path in the list

            # [2] read images and detect image events::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
            raw_imgs = {}
            for i in range(n):
                raw_imgs[i] = utils.read_crop_resize(C[sid][deploy][i][-1],height=height,width=width)
                # stores images to the dictionary
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description=des.lstrip(" "),formatter_class=argparse.RawTextHelpFormatter)
        # this line starts an argument parser for the command line
    parser.add_argument('--in_dir',type=str,help='input directory of images\t[None]') # input directory of images
    parser.add_argument('--out_dir',type=str,help='output directory prefix\t[None]') # output directory 
    parser.add_argument('--sids',type=str,help='specific comma seperated sids to process\t[all]') # site ID
    parser.add_argument('--res',type=str,help='comma seperated output width,height\t[600,200]') # resolution of images
    parser.add_argument('--mean',type=float,help='luma and hue mean applied during enhancement\t[1.0]') # luma and hue mean applied during enhancement
    parser.add_argument('--sharp',type=float,help='sharpness applied after luma before enhancement\t[2.0]') # sharpness applied after luma enhancement
    parser.add_argument('--enh_hrs',type=int,help='temporal based NN enhancement hours\t[2]') # WHAT IS THIS
    parser.add_argument('--agg_hrs', type=int, help='number of hours per day to aggregate\t[2]') # WHAT IS THIS
    parser.add_argument('--equalize',action='store_true',help='luma channel histogram equalization\t[False]') # equalize luma channel histogram
    parser.add_argument('--json_img_map',action='store_true',help='generate a temporal weekly json image map\t[False]') # WHAT IS THIS
    parser.add_argument('--label_sub_folders',action='store_true',help='generate subfolders for passed images\t[False]') # generate subfolders for passed images
    parser.add_argument('--advanced',action='store_true',help='luma differential corrected by dense optical flow\t[False]') # luma differential corrected by dense optical flow
    parser.add_argument('--winsize',type=int,help='kernel window for optical flow if using advanced\t[2]') # WHAT IS THIS
    parser.add_argument('--cpus',type=int,help='CPU cores to use for || processing\t[1]') # CPU cores to use for processing
    args = parser.parse_args()
    
    #-REQUIRED ARUGMENTS------------------------------------
    if args.in_dir is not None: # INPUT DIR
        in_dir = args.in_dir 
    else: raise IOError
    if args.out_dir is not None: # OUTPUT DIR
        out_dir = args.out_dir
    else: raise IOError
    if args.res is not None:  # OUTPUT RESOLUTION
        width,height = [int(r) for r in args.res.split(',')] 

    #-OPTIONAL ARGUMENTS------------------------------------
    if args.sids is not None: # SITE ID
        sids = [int(sid) for sid in args.sids.split(',')] 
    if args.mean is not None:
        mean = args.mean
    else: mean = 1.0 # if value not set, set the value
        
    if args.sharp is not None:
        sharp = args.sharp
    else: sharp = 2.0 # if value not set, set the value
        
    if args.enh_hrs is not None:
        enh_hrs = args.enh_hrs
    else: enh_hrs = 0 # if value not set, set the value
        
    if args.agg_hrs is not None:
        agg_hrs = args.agg_hrs
    else: agg_hrs = 0 # if value not set, set the value
        
    if args.equalize:
        equalize = True
    else: equalize = False # if value not set, set the value
        
    if args.advanced:
        advanced = True
    else: advanced = False # if value not set, set the value
        
    if args.winsize is not None:
        winsize = args.winsize
    else: winsize = 5 # if value not set, set the value
        
    if args.cpus is not None:
        cpus = args.cpus
    else: cpus = 1 # if value not set, set the value
    #-OPTIONAL ARGUMENTS------------------------------------

    
    # creating a dictionary with the arg.parse parameters
    ...
    # obtains the SIDs of all JPGs
    raw_path = in_dir+'/*/*.JPG'
    raw_paths = sorted(glob.glob(raw_path),key=get_sid)
    
    # locate unlabeled JPGs if the file name did not contain SIDs
    if len(raw_paths)<=0:
        print('raw image data is not ordered by label, proceeding to locate unlabeled JPGs...')
        raw_paths = sorted(glob.glob(in_dir+'/*.JPG')) # attempting to search again
        print('located %s unlabeled image paths'%(len(raw_paths)))
    else: print('located %s label ordered image paths'%len(raw_paths))

    # manipulate the out_dir string to create a new directory path
    # it appends a suffix based on the enhacement hours, aggregation hours and whether the processing is advanced or basic
    # if the directory does not exist, then creates it
    # prints out the location of the newly created output directory
    ...
    S = {} # new dictionary 
    for path in raw_paths: # iterate each path in raw_paths
        sid,deploy = get_sid(path),get_deploy(path) # extracts the deploy and the sid 
        if sid in S:
            if deploy in S[sid]: S[sid][deploy] += [path] # check if deploy exist for that sid, if it does the path is appended to the exististing 
            else:                S[sid][deploy]  = [path] # list of paths for that deploy, OTHERWISE, a new list is created for that deploy
        else:                    S[sid]  = {deploy:[path]}# if the list does not exist in S, a new entry is created with the deploy and path 
    # basically S is a nested dictionary where the outer 
    # keys represents sid, and each inner dictionary contains
    # keys corresponding to different deploy values along
    # with lists of associated path values

    # stores the sorted list of S keys (sids)
    ss = sorted(S)
    if args.sids is None: sids = ss 
        # if we did not specify sids, then ss is sids
    for s in ss:
    ...
    print('%s total deployments were found among the image paths'%(sum([len(S[sid]) for sid in S])))
        # any site id found through glob not specified in the arg.parse sid, gets removed
    
    # [S] read exif and time sort with temporal trigger removal routine
    O = temporally_order_paths(S) 
        # This functions read all images exif data to order and partition triggered events,returns: order,error,trigs
    for sid in O: # iterates through each sid in O
        for deploy in O[sid]: # from each sid iterate through each deployment
            if len(O[sid][deploy]['trigs'])>0: # [0] remove triggered events
                ...
                print('copying %s triggered images to the trigs folder:%s'%(len(O[sid][deploy]['trigs']),trigs_dir)) # creating new folder
                for r in O[sid][deploy]['trigs']:
                    img_name = trigs_dir+'/S%s_D%s_I%s.JPG'%(sid,deploy,trig)
                    cv2.imwrite(img_name,cv2.imread(r[-1])) # the images are saved
                    trig += 1
            if len(O[sid][deploy]['exif'])>0: #[0] remove exif events
                ...
                print('copying %s images with non-valid dates to the exif folder:%s'%\
                      (len(O[sid][deploy]['exif']),exif_dir)) # creating new folder
    ...
    # Initializing empty list and i is list of cpus 
        # H will store tuples containing the length of an order and the order itself.
        # P is a dictionary with keys corresponding to CPU indices (ranging from 0 to cpus-1).
        # T is an empty dictionary that will store information about images.

    for sid in O:
        for deploy in O[sid]:
            H += [(len(O[sid][deploy]['order']),O[sid][deploy]['order'])] # contains the length of an order and the order itself
    H = sorted(H,key=lambda x: x[0])[::-1]
    
    for i in range(len(H)): P[i%cpus] += [H[i]]
        # the list H is divided among the available CPUs
        # each CPU in P will store a subset of orders from H

    for cpu in P: # for each CPU
        
        T[cpu] = {'n':0,'imgs':{}} # n number of images, and image information
        for d in range(len(P[cpu])): # for each order in the CPU's partition
            if P[cpu][d][0]>0: # sids can be duplicates, deploys are unique here...
                sid,deploy = get_sid(P[cpu][d][1][0][-1]),get_deploy(P[cpu][d][1][0][-1]) 
                    # extract the sid and deploy from the last item in the order
                if sid not in T[cpu]['imgs']: T[cpu]['imgs'][sid] = {}
                    # if the sid is not already in T[cpu]['img'], create an entry for it
                T[cpu]['imgs'][sid][deploy] = P[cpu][d][1]
                    # store the order information under the deploy key for that sid
                T[cpu]['n'] += P[cpu][d][0]
                    # update the total image count for that CPU
    n_images = sum([T[cpu]['n'] for cpu in T])
    print('partitioned %s total images to %s processors'%(n_images,cpus))
    """

    process_image_partitions located here
    
    """
    start = time.time()
    p1 = mp.Pool(cpus) # multiprocessing pool is created
    for cpu in T:  # balanced sid/deployments in ||
        print('dispatching %s images to core=%s'%(T[cpu]['n'],cpu)) # iterate over each cpu in the list
        p1.apply_async(process_image_partitions, # asynchronously applies process_image_partitions to the images associated with the current CPU.
                       args=(T[cpu]['imgs'],params),
                       callback=collect_results) # ensures that the collect_results function is called when the processing is complete.
        time.sleep(0.1)
    p1.close() # closed
    p1.join()  # line waits for all processes in the pool to finish.
    stop  = time.time()
    ...
    if args.json_img_map:
        ...
        generate_img_map_json(out_dir,out_dir+'/img_map.json') # CALLS OUT A DIFFERENT FUNCTION

    if args.label_sub_folders:
        I = {}
        for img_path in glob.glob(out_dir+'/passed*/*.JPG'): # iterates over images within the passed directory
