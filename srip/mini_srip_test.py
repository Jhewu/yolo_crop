...
"""ADDED BY JUN TO PERFORM MULTI-CROP"""
def multi_crop(image, amount_to_crop):  
    quadrants = int(np.sqrt(amount_to_crop))
    if quadrants ** 2 != amount_to_crop: 
        print(f"\nThe amount_to_crop, does not yield perfect square, defaulting to 4")
        amount_to_crop = 4
    ...
    crop_height = height // int(np.sqrt(amount_to_crop))
    crop_width = width // int(np.sqrt(amount_to_crop))
    
    # Initialize a list to store the cropped images
    crops = []
    
    # Split the image into 'amount_to_crop' equal-sized crops
    for i in range(int(np.sqrt(amount_to_crop))):
        for j in range(int(np.sqrt(amount_to_crop))):
            x1, y1 = j * crop_width, i * crop_height
            x2, y2 = min(x1 + crop_width, width), min(y1 + crop_height, height)
...
"""MODIFIED BY JUN"""
def worker_image_partitions(C, out_dir, width=600, height=200, model_dir=f"model/best.pt", window_size=32, conf=0.7, amount_to_crop=4):
    # Load the YOLO model
    ...
    for sid in C:
        for deploy in sorted(C[sid]):
            ...
            if coords is not None:
                crops = [cropImageWithCenter(cv2.imread(path), coords) for path in paths]
                mutli_crops = [multi_crop(image, amount_to_crop) for image in crops]
                cropped_images = [crop for sublist in mutli_crops for crop in sublist]
            else:
                cropped_images = [cv2.imread(path) for path in paths]

            print('processing %s paths for sid=%s, deploy=%s'%(len(paths),sid,deploy))

            print(f"\nComparing the prev length {len(paths)} to after length {len(cropped_images)}")
